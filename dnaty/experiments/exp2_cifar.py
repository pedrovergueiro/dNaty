...
N_POP = 8
T_LOCAL = 3
BATCH_SIZE = 128   # CIFAR CNN: batch menor para caber em RAM
CIFAR_TRAIN_SUBSET = 10000  # 10K em RAM — rápido e representativo
RESULTS_DIR = "results"
...
def local_train_cnn_fast(ind, fast_ds, n_epochs, lr, device, batch_size=256):
    """Treino CNN com FastDataset — múltiplos batches por epoch, zero I/O."""
    model = ind.model.to(device)
    model.train()
...
def train_resnet_fast(model, fast_ds, n_epochs=20, device='cpu', batch_size=256):
    """Treina ResNet-8 com FastDataset."""
...
def run_dnaty_cnn_seed(seed, device):
    ...
    for gen in tqdm(range(1, N_GENERATIONS + 1), desc=f"CIFAR seed={seed}"):
        ...
        for ind in mutated:
            lb, la, gn = local_train_cnn_fast(ind, fast_ds, T_LOCAL, 1e-3, device, BATCH_SIZE)
...
def main():
    ...
    for seed in SEEDS:
        ...
        print("  [ResNet-8 — FastDataset 10K]")
        torch.manual_seed(seed)
        fast_ds_full = FastDataset('CIFAR10', device=device, train_subset=CIFAR_TRAIN_SUBSET)
        resnet = ResNet8()
        acc_r = train_resnet_fast(resnet, fast_ds_full, n_epochs=20, device=device)
